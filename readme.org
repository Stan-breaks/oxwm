...
#+begin_src sh
sudo pacman -S zig libx11 libxft freetype2 fontconfig pkg-config
#+end_src
...
sudo cp zig-out/bin/oxwm /usr/local/bin/
#+end_src
...
└── tonarchy-config.lua                  [Alternative preset configuration]
#+end_src
